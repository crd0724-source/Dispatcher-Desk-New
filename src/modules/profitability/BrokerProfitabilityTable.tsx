...
import { BrokerProfitability } from './profitabilityTypes.ts';
import { StatusBadge } from '../../components/common/StatusBadge.tsx';
import { formatCurrency, formatMiles, formatRPM } from '../../lib/calculations.ts';

interface BrokerProfitabilityTableProps {
  brokersData: BrokerProfitability[];
}

type SortField = 'broker' | 'loads' | 'gross' | 'miles' | 'rpm' | 'profit' | 'margin';

export const BrokerProfitabilityTable: React.FC<BrokerProfitabilityTableProps> = ({ brokersData }) => {
  ...
  const sortedData = useMemo(() => {
    return [...brokersData].sort((a, b) => {
      let comparison = 0;
      switch (sortField) {
        case 'broker':
          comparison = a.broker.company_name.localeCompare(b.broker.company_name);
          break;
        case 'loads':
  ...
  if (brokersData.length === 0) {
    return (
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-12 text-center">
        ...
        <h4 className="text-xs font-semibold text-slate-300">No Broker Data</h4>
        <p className="text-[11px] text-slate-500 mt-1">No broker loads found in this timeframe.</p>
      </div>
    );
  }
      ...
      <div className="p-4 border-b border-slate-800 flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <h3 className="text-xs font-bold text-slate-200 uppercase tracking-wider">
            Freight Broker Profitability & Credit Matrix ({brokersData.length})
          </h3>
        </div>
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-left border-collapse text-xs">
            ...
            <tr className="border-b border-slate-800 bg-slate-950/70 text-[11px] font-semibold text-slate-400 uppercase tracking-wider select-none">
              ...
              >
                <div className="flex items-center gap-1.5">
                  <span>Brokerage Entity</span>
                  {renderSortIcon('broker')}
                </div>
              </th>
              <th className="py-3 px-3 whitespace-nowrap">Credit Status</th>
              <th
          ...
          <tbody className="divide-y divide-slate-800/60 font-medium">
            {sortedData.map((item) => (
              <tr key={item.broker.id} className="hover:bg-slate-800/40 transition-colors">
                <td className="py-2.5 px-4 whitespace-nowrap">
                  <div className="flex flex-col">
                    <span className="font-semibold text-slate-100">{item.broker.company_name}</span>
                    <span className="text-[10px] text-slate-400 font-mono">
                      MC #{item.broker.mc_number || 'N/A'} • Pay: {item.broker.payment_terms_days || 30} days
                    </span>
                  </div>
                </td>
                <td className="py-2.5 px-3 whitespace-nowrap">
                  <StatusBadge status={item.broker.credit_status} type="credit" size="sm" />
                </td>
