-- ====================================================================
-- DispatcherDesk Database Migration: Billing Paid Period Invariant Protection
-- File: supabase/migrations/20260907000006_billing_paid_period_invariant.sql
-- Description:
-- Updates public.apply_subscription_payment_transition to protect the
-- billing period invariant: when transitioning from trialing or trial_expired
-- to active, if paid period dates are omitted, do not preserve the 14-day
-- trial period. Use NOW() + INTERVAL '1 month' as the database fallback.
-- ====================================================================
...
BEGIN
  ...
  -- 4. Calculate effective billing state and period dates
  v_updated_state := COALESCE(p_new_billing_state, v_sub.billing_state);

  -- Paid activation invariant: When transitioning from trialing or trial_expired to active,
  -- if paid period dates are omitted, do not preserve the 14-day trial period.
  -- Use NOW() + INTERVAL '1 month' as the database fallback.
  IF v_sub.billing_state IN ('trialing', 'trial_expired') AND v_updated_state = 'active' THEN
    v_period_start := COALESCE(p_current_period_start, NOW());
    v_period_end := COALESCE(p_current_period_end, NOW() + INTERVAL '1 month');
  ELSE
    v_period_start := COALESCE(p_current_period_start, v_sub.current_period_start);
    v_period_end := COALESCE(p_current_period_end, v_sub.current_period_end);
  END IF;

  -- 5. Record Payment Transaction (idempotent via uq_billing_tx_provider_payment)
  ...
  -- 7. Apply atomic subscription updates
