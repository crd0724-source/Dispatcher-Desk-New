-- ====================================================================
-- DispatcherDesk Migration: Team Invitation Concurrency Hardening (S5.2.1)
-- Version: 1.0.8
-- File: supabase/migrations/20260830000008_team_invitations_concurrency_hardening.sql
-- Description:
-- 1. Hardens public.create_team_invitation with safe exception handling around
--    the invitation INSERT against race conditions, catching unique_violation
--    on idx_org_invitations_active_org_email and returning a controlled business
--    exception ('An active pending invitation already exists for this email.').
-- 2. Hardens public.accept_team_invitation with safe exception handling around
--    the organization_members INSERT, catching unique_violation on unique_org_user
--    and returning a controlled business exception ('You are already a member of this organization.').
-- 3. Ensures strict transactional integrity: no partial acceptance, no acceptance if
--    membership creation fails, no privilege escalation or role overwrite.
-- 4. Preserves all existing RPC signatures, SECURITY DEFINER attributes, search paths,
--    and RLS permission grants.
-- ====================================================================

-- 1. RPC: create_team_invitation (Concurrency Hardened)
...
BEGIN
  ...
  -- 9. Pre-insert rejection if an active pending invitation already exists
  IF EXISTS (
    SELECT 1
  ...
  -- 10. Insert the new invitation record with safe unique_violation exception block for concurrency
  BEGIN
  ...
  EXCEPTION
    WHEN unique_violation THEN
      ...
      IF v_constraint_name = 'idx_org_invitations_active_org_email' 
...
-- 4. RPC: accept_team_invitation (Concurrency Hardened)
...
BEGIN
  ...
  -- 8. Insert new organization membership record with safe exception block for concurrency
  BEGIN
  ...
  -- 9. Mark invitation accepted atomically (only reached if membership insertion succeeded)
  UPDATE public.organization_invitations
  SET
    accepted_at = NOW(),
    updated_at = NOW()
